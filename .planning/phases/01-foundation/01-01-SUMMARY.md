...
plan: 01
subsystem: testing
tags: [pytest, fastapi, sqlalchemy, sqlite, testclient, httpx, bcrypt, pwdlib]
...
affects:
  - 01-02 (shop/ package must satisfy conftest imports: create_app, Base, get_db, hash_password, User, ShopConfig)
  - 01-03 through 01-08 (all subsequent plans use test infrastructure created here)
...
tech-stack:
  added:
    ...
    - sqlalchemy 2.0 (ORM)
    - pwdlib[bcrypt] 0.3.0 (password hashing, replaces unmaintained passlib)
    - bcrypt>=4.0.0,<5.0.0 (pinned — bcrypt 5.0 removed __about__ used by pwdlib)
    - itsdangerous (session signing)
    - httpx (FastAPI TestClient async transport)
  patterns:
    - In-memory SQLite with StaticPool for hermetic per-function test isolation
    - FastAPI dependency_overrides for DB injection in tests
    - xfail(strict=False) stubs for scaffold-before-implementation TDD wave approach
...
requirements-completed: [AUTH-01, AUTH-02, AUTH-03, AUTH-04, AUTH-05, AUTH-06, SETUP-01, SETUP-02, SETUP-03, SETUP-04]
