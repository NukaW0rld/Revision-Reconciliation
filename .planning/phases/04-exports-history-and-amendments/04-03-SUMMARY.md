...
plan: "03"
subsystem: exports
tags: [work-order, exports, weasyprint, csv, pdf, partial-fai]
dependency_graph:
  requires: [04-01, 04-02]
  provides: [work-order-pdf, work-order-csv, work-order-routes, work-order-template]
  affects: [shop/services/exports.py, shop/routers/exports.py, shop/templates/exports/work_order.html, shop/templates/runs/status.html]
tech_stack:
  added: []
  patterns: [WeasyPrint HTML template, CSV DictWriter, StreamingResponse attachment, effective classification helper]
key_files:
  created:
...
decisions:
  - _effective_classification() helper centralizes override_classification vs pipeline_classification resolution for work order and future features
  - Drawing Reference column uses "Balloon {char_no}" format — bbox-to-zone mapping deferred to future iteration (zone requires page dimensions not stored)
  - CSV-only route tested in automated tests; PDF route coverage deferred to Docker e2e in Plan 06
  - test_work_order_button_visible requires _login_engineer() call — plan's stub omitted login, fixed as Rule 1 auto-fix
metrics:
  duration: "4 min"
