...
- **Setup completion signal:** `setup.py` logs `INFO` on `step2_post` completion: `"Setup wizard complete: admin password set, setup_complete=True"`. Grep: `grep "Setup wizard complete" app.log`.
- **Redirect audit:** `/setup/step3` and `/setup/step4/*` return 302 with `Location: /login` (complete) or `/setup/` (incomplete). Inspect with `curl -I http://host/setup/step3`.
- **DB state inspection:** `SELECT setup_complete, wizard_step FROM shop_config WHERE id=1;` — post-setup: `setup_complete=1, wizard_step=2`.
- **Guard middleware:** `SetupGuardMiddleware` reads `setup_complete` from DB each request. If misconfigured, all routes redirect to `/setup/step1`. Check middleware logs or DB state.
- **Failure surface:** If `step2_post` fails (e.g. DB error), the exception propagates as 500. No partial-commit state: `setup_complete` and `wizard_step` are set in the same `db.commit()` call.
- **Redaction:** No secrets are logged. The admin password hash is never emitted in any log statement.
...
- [x] **T01: Strip wizard to 2 steps and update guard logic** `est:45m`
...
- [x] **T02: Add validate-xlsx endpoint and per-run column mapping partial** `est:45m`
...
- [x] **T03: Remove column mapping from admin settings** `est:15m`
...
- [x] **T04: Update test suite for new wizard and per-run mapping** `est:45m`
